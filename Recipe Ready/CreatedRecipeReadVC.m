...
#import "CreatedRecipeReadVC.h"
#import "ReadOptionCell.h"

@interface CreatedRecipeReadVC ()
{
	NSArray* arrOptions;
	int nSelectedIndex;
}
...
@implementation CreatedRecipeReadVC

- (void)viewDidLoad {
	[super viewDidLoad];
	// Do any additional setup after loading the view.
	arrOptions = @[@"Everyone", @"Friend Only", @"None"];
	nSelectedIndex = (int)[arrOptions indexOfObject:appdata.strCreateRecipeOption];
...
- (IBAction)backAction:(id)sender {
	[[[FIRUsers child:appdata.me.snapKey] child:kStrCreatedRecipeOption] runTransactionBlock:^FIRTransactionResult * _Nonnull(FIRMutableData * _Nonnull currentData) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	appdata.strCreateRecipeOption = arrOptions[indexPath.row];
	[self.readOptionTblView reloadData];
}
