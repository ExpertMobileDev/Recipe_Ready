...
- (void)refreshPostedData {
	[FIRPostedRecipe observeEventType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
		...
		NSArray *sortedArray = [arrPosted sortedArrayUsingComparator:^NSComparisonResult(RecipeDetail *p1, RecipeDetail *p2){
			
			return [p1.title compare:p2.title];
			
		}];
		arrPosted = [sortedArray mutableCopy];
...
- (void)getRecipeDetail:(int) index {
	[appdata.me getRecipeDetail:index handler:^(NSDictionary *result, NSString *errorMsg) {
		...
		if (result) {
			...
			for (RecipeDetail* aDetail in appdata.arrPosted) {
				if (self.nId == aDetail.index) {
					recipeDetail = aDetail;
					break;
				}
			}
			...
			if (recipeDetail) {
				for (UserInfo* aUser in arrUserInfos) {
					if ([aUser.user_id isEqualToString:recipeDetail.userid]) {
		...
		arrIngredients = recipeDetail.arrIngredients;
		...
		if (![AppData isNullOrEmptyString:recipeDetail.instructions])
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
	...
	if ([segue.identifier isEqualToString: @"PostedData"]) {
		...
		vcDetail.strUserId = recipeDetail.userid;
	}
}

- (IBAction)showUsersPostedData:(id)sender {
	if ([recipeDetail.userid isEqualToString:@""])
		return;
